...
- [ ] T005 [SETUP] Create `server/index.ts` entry point: Express + WebSocket upgrade handler, static file serving for client build, PostgreSQL connection via `postgres` driver. **On startup**: (1) run Drizzle `migrate()` to auto-apply pending migrations, (2) zombie deploy triage — force-fail all `running` deployments with `errorMessage: "Interrupted by dashboard restart"`
...
- [ ] T012 [BE] Implement auth middleware in `server/middleware/auth.ts`: session-based auth, bcrypt password verify against `ADMIN_PASSWORD_HASH` env var, session stored in PostgreSQL. Routes: `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`. Cookie: httpOnly, secure, sameSite strict
...
- [ ] T023 [BE] [US1] Implement deployment routes in `server/routes/deployments.ts` with Zod validation: POST /api/apps/:appId/deploy (pre-flight checks → acquire lock → **try** { run deploy.sh via script-runner → stream to job → record in DB } **finally** { release lock }). Lock release MUST happen in finally block regardless of DB errors. GET /api/apps/:appId/deployments (history with pagination), GET /api/deployments/:id
- [ ] T024 [BE] [US2] Implement rollback route in `server/routes/deployments.ts`: POST /api/apps/:appId/rollback (acquire lock → **try** { run rollback.sh → stream → record } **finally** { release lock }). Include target commit selection
