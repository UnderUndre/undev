...
- **Gemini PR #11 review comments (2026-04-25)** — 2 inline comments pulled from github.com/UnderUndre/undev/pull/11, both applied (spec v1.2.1):
  1. **quickstart.md:71** (medium) — hardcoded `origin` remote in the example deploy script. Fixed: introduced `REMOTE="${DEPLOY_REMOTE:-origin}"` pattern so operators on `upstream`/`prod`/etc. can override via env var without patching the template.
  2. **spec.md FR-003** (security-medium + medium) — blacklist missing `(` and `)`. Parens are shell metacharacters (subshell delimiters); `$(...)` already closed via `$` rejection but solo `(` / `)` stayed valid, creating threat-model inconsistency. Fixed: added both chars to the FR-003 banned set + propagated to plan.md §Shared validator rule 6 + tasks.md T003 (validator impl) + T004 (3 new fixture cases: `foo(bar`, `foo)bar`, `(cmd)`) + T018 (2 new integration cases: `scripts/(cmd).sh`, `scripts/foo(1).sh`).
- Both Gemini-PR11 findings are defence-in-depth — neither is exploitable through the dispatch path (`shQuote` neutralises both classes downstream), but both close intake-level inconsistencies that weaken the threat-model story if/when `shQuote` gets refactored. Cost: 2 regex chars + 1 env-var lookup. Benefit: consistency + operator-friendly template.
