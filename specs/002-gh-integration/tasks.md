...
- [ ] T002 [DB] Add `githubConnection` table to `server/db/schema.ts` per data-model.md: id (text, PK, CHECK='DEFAULT'), token, username, avatarUrl, tokenExpiresAt (text), connectedAt (text). Add `githubRepo` (text, nullable) column to existing `applications` table
- [ ] T003 [DB] Create migration `server/db/migrations/0002_github.sql`: CREATE TABLE github_connection with CHECK constraint, ALTER TABLE applications ADD COLUMN github_repo. Update `meta/_journal.json`
...
- [ ] T006 [BE] Implement GitHub proxy routes in `server/routes/github.ts` with Zod validation: GET /api/github/repos?q= (search repos via github service, require q min 2 chars), GET /api/github/repos/:owner/:repo/branches, GET /api/github/repos/:owner/:repo/commits?branch=&count=20. All routes: check github connection exists first → 400 GITHUB_NOT_CONNECTED if missing. Handle rate limit → 429 GITHUB_RATE_LIMITED
