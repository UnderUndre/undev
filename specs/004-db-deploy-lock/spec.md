...
- **FR-012**: `checkLock(serverId)` MUST be **read-only** AND filter out orphans inline. Query: `SELECT app_id FROM deploy_locks WHERE server_id = $1 AND dashboard_pid IN (SELECT pid FROM pg_stat_activity) LIMIT 1`. Returns `app_id` or `null`. No mutations, no advisory lock probing. The JOIN against `pg_stat_activity` ensures `checkLock` never returns a stale owner after a rolling-deploy crash: if the old dashboard's backend is gone, the row is implicitly filtered out even before startup reconciliation runs on the new dashboard. Orphan metadata is still eventually DELETE'd by FR-022 (startup) or overwritten by the next `acquireLock` (ON CONFLICT DO UPDATE) — FR-012 just stops consumers from seeing ghosts in the meantime.

### Crash Safety

- **FR-020**: If the dashboard process dies while holding the lock, Postgres MUST release the advisory lock automatically on connection termination. No user action required. **This releases the dashboard-side coordination only — a deploy process that was already running on the target server via SSH is NOT interrupted by the dashboard's crash** (see § Out of Scope for rationale). FR-020 guarantees "no two dashboard-initiated deploys for the same server run concurrently"; it does not guarantee "at most one deploy script is executing on the target host at any time".
...
- **FR-025 (pool-exhaustion watchdog)**: A background timer MUST periodically inspect `held` (the in-memory `Map<serverId, ReservedSql>`) and forcibly release entries whose `acquiredAt` is older than `DEPLOY_LOCK_MAX_AGE_MS` (default 30 minutes, configurable via env). Without this watchdog, a stuck deploy (hung SSH, unhandled promise, lost network-to-target) would leak one pooled DB connection per incident — 10 incidents exhaust the default postgres pool and brick the dashboard. Watchdog action on timeout: log at `warn` with `{ serverId, ageMs, appId }`, call `releaseLock(serverId)` unconditionally, increment a metric counter. Interval: 60 seconds. Justification for 30 min cap: largest known deploy takes ~12 min; 2.5× margin catches hangs without false positives.
...
- Lock timeouts / automatic takeover **beyond** the FR-025 watchdog. The watchdog forcibly releases locks older than `DEPLOY_LOCK_MAX_AGE_MS` to prevent pool exhaustion, but it does NOT gracefully interrupt the deploy script — that's admin territory.
- **Interrupting target-host deploy scripts when the dashboard crashes.** The DB lock protects dashboard-side coordination ("no two deploys from the dashboard UI at the same time"), NOT target-host execution. If dashboard dies mid-deploy, the SSH-launched script continues on the target host on its own; Postgres releases our advisory lock; second admin clicks Deploy → second script starts on the SAME target, racing the first. This is a known gap inherited from the pre-feature-004 design — fs-lock had the same property because the script didn't hold the lock, the dashboard did. True mutual exclusion at the target-host level requires either (a) the deploy script itself to take a local lock (e.g. `flock /tmp/deploy.lock`) which ai-digital-twins already does, or (b) a target-side daemon that rejects overlapping requests. Both out of scope for this feature; documented for future consideration.
- Multi-instance dashboard HA. The feature assumes one dashboard process per database. A multi-instance setup would share the same Postgres advisory lock namespace automatically — but the `deploy_locks` row's `dashboard_pid` would stop being unique across instances and `checkLock`'s `pg_stat_activity` filter would need to account for multiple dashboard backends. Deferred.
...
- **Direct connection to Postgres, no transaction-mode pooler.** If a future deployment puts PgBouncer (or Odyssey / RDS Proxy) between the dashboard and Postgres, it MUST run in `session` pool mode. `transaction` mode breaks session-level advisory locks: `pg_try_advisory_lock` binds to a backend, but the pooler reassigns each statement to a new backend, so `pg_advisory_unlock` reaches the wrong session and the original lock orphans for the session's natural lifetime (up to backend idle timeout — hours). A dashboard startup self-check SHOULD verify `SHOW pool_mode` returns `session` or errors cleanly (implementation detail in plan).
- Connection pool can spare 1–N connections for lock-holding. Default `postgres` pool size (10) comfortably supports the expected fleet size (~5 servers). Stuck-deploy scenarios that would otherwise leak connections are bounded by FR-025 watchdog.
