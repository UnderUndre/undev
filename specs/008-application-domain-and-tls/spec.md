...
- Q: Hard delete sequence (revoke → rm files → API → DB) is fragile — if Caddy is dead at delete-time, the app row stays in DB forever. → A: **FR-018a — hard delete is best-effort on Caddy steps, authoritative on DB steps.** Caddy interactions wrapped in try/catch, failures logged + audited (`app_cert_events.event_type = 'hard_delete_partial'`), but DELETE app row always proceeds. A "ghost-app" that can't be removed because the server is unreachable is worse UX than orphaned cert files (a daily reconciler picks them up). Operators see the partial-delete audit row and can SSH in to clean up if needed.
- Q: DNS pre-check with `mismatch` for servers behind NAT / Load Balancer (AWS ELB, GCP LB) — every issuance triggers an unnecessary warning. → A: **FR-014 warning text explicitly names the LB/NAT case as legitimate.** The "Try anyway" checkbox stays — but the warning body now says: "DNS points to X, server is at Y. If your server is behind a Load Balancer or NAT, this is expected." Removes false-alarm noise for cloud deployments without weakening the actual mistake-detection signal.
- Q: 5-minute drift-detection cron racing with operator domain change → stale-write to Caddy. → A: **Row-level lock on the `applications` row, held through both reads and Caddy PUT.** Reconciler and operator-handler acquire the same lock; second writer waits, then re-reads fresh state. Edge case documented; implementation is `SELECT ... FOR UPDATE` in the reconcile transaction.
- Q: Caddy admin port 2019 already taken on a target → `setup-vps.sh` Caddy install fails silently? → A: **No, it MUST detect with `ss -ltn` and bail with actionable error.** Mentioned in Edge Cases. v1 does not auto-discover an alternate port; that's a v2 nicety.
- Q: IDN domains (`münchen.de`, etc.) — does the regex accept them? → A: **No, accepts only punycode (`xn--mnchen-3ya.de`).** Form helper text instructs the operator. Auto-conversion of Unicode → punycode is v2.

### Session 2026-04-28 (GPT review pass)

- Q: Operator clicks "Try anyway" on DNS mismatch with the explicit reason "I just updated the A-record, propagation is in progress". 30 seconds later the dashboard hits Let's Encrypt, ACME challenge fails because DNS still points wrong, rate-limit slot wasted. → A: **FR-014a — second resolution at T+120 seconds before initiating ACME.** The 2-minute pause matches typical "fresh A-record" propagation windows; if it's still mismatched at T+120, the operator's reason was wrong and we abort before burning the rate-limit slot. Cancel-during-wait supported.
- Q: When `applications.domain` goes from `foo.example.com` to NULL (operator clears it), what happens to the Caddy site? → A: **FR-017a — reconciler treats `domain = NULL + proxy_type = 'caddy'` as a removal trigger.** Without explicit handling, the site lingers in Caddy serving HTTPS for an app the operator considers "domain-less". Cert row marked orphaned with the same grace-period treatment as a rename.
- Q: Should the domain UNIQUE constraint deduplication ALSO check soft-deleted apps to prevent re-use of an in-grace-period domain? → A: **No, soft-delete already moves the cert to `orphaned` status and the app row is excluded from active queries.** Re-using a domain whose previous app was soft-deleted is fine — the new cert lifecycle is independent. The 30-day rate-limit retention on the orphaned cert is a Let's Encrypt concern, not a UNIQUE-constraint concern.
...
- **Caddy admin port 2019 already taken** on the target (legacy Caddy install, another local daemon, custom firewall config): `setup-vps.sh` Caddy install step MUST detect the port conflict (`ss -ltn '( sport = :2019 )'`) before launching Caddy. If occupied, bail with actionable error "Port 2019 is already in use by `<pid>` (`<process-name>`). Stop the existing process or set `CADDY_ADMIN_PORT` env var to a different port (UFW rule must be re-checked: 2019 is loopback-only — see FR-028)." Out of scope: dynamic admin-port discovery — it complicates SSH tunnelling and gives operators a moving target.
- **Drift-detection cron races with operator domain change**: 5-minute reconciler reads desired-state at T+0, operator changes domain at T+30s, reconciler PUTs Caddy config at T+45s using stale read. Result: Caddy serves OLD domain even though DB has NEW. Mitigation: reconciler MUST acquire a row-level lock (`SELECT ... FOR UPDATE` on the app row) before reading desired-state and MUST hold it through the Caddy PUT. Operator's domain-change handler also acquires the same lock — second writer waits, then re-runs reconciliation with fresh state. Drift-detection becomes safe by serialisation, not by optimistic-locking version columns.
- **IDN / Punycode domains** (`münchen.de`, encoded as `xn--mnchen-3ya.de`): v1 accepts ONLY the punycode form in the form input (regex `^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?(\.[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)+$` already enforces this — a-z is post-punycode). Form helper text: "Use punycode for international domains (e.g. `xn--mnchen-3ya.de`, not `münchen.de`)." Unicode-input auto-conversion to punycode is a v2 nicety.
...
- **FR-014**: `nxdomain` MUST block issuance with hard error. `match` MUST proceed silently. `cloudflare` and `mismatch` MUST surface a warning in the form with an "I know, try anyway" checkbox. Without the checkbox, issuance does not start. Warning text for `mismatch` MUST acknowledge the legitimate-mismatch case explicitly: *"Domain X resolves to <resolved-ip>, but this server's address is <server.host>. Common reasons: (1) DNS still propagating — wait a few minutes and retry; (2) wrong A-record — fix in your DNS provider; (3) **server is behind a Load Balancer, NAT, or cloud-provider front-end** (AWS ELB/ALB, GCP Load Balancer, etc.) — in this case the mismatch is expected and 'Try anyway' is safe. If unsure, ask your network operator before continuing."* Surfacing the LB/NAT case in the message body removes the false-alarm noise for cloud deployments.
- **FR-014a**: When the operator submits with "Try anyway" on a `mismatch` outcome (NOT `cloudflare` — that path is its own LB-tolerant story), the dashboard MUST perform a **second DNS resolution at T+120 seconds** before initiating the ACME challenge. The 2-minute pause gives DNS propagation a real chance — the most common operator reason for clicking "Try anyway" is "I just updated the A-record". If the second resolve still does NOT include `server.host`, issuance MUST abort with a hard error "DNS still mismatched after 2-minute propagation wait — fix the A-record before retrying." This guards a Let's Encrypt rate-limit slot from being burned on a propagation-not-yet scenario. The cert row stays `pending` during the 2-minute wait; UI surfaces a "Validating DNS… (~2 min)" indicator. If the operator cancels during the wait, cert row goes to `failed` with `error_message = 'cancelled by operator during DNS revalidation'`.
...
- **FR-017a**: When `applications.domain` is set to NULL (operator clears the domain — "this app no longer needs a public hostname"), the reconciler MUST: (a) mark the existing `app_certs` row `status = orphaned, orphan_reason = 'domain_change'` (same grace-period treatment as a rename — 7 days for rollback); (b) **remove the Caddy site config** for that app via admin API. Without (b), the Caddy site lingers as a "dangling site" — operator clears the domain in the UI but Caddy still serves the app over HTTPS until the next manual cleanup. Reconciler MUST treat `domain = NULL + proxy_type = 'caddy'` as a removal trigger, not a no-op.
- **FR-018**: Soft app delete (default) MUST mark all `app_certs` rows for that app as `orphaned` with `orphan_reason = 'app_soft_delete'`. Hard app delete (explicit "Remove from server") MUST attempt: (1) ACME-revoke each cert (mechanism: Caddy admin API site removal + invocation of Caddy's revoke flow — exact endpoint shape determined in plan/research, NOT prescribed here as a literal path), (2) remove cert files from Caddy storage, (3) remove the Caddy site config, (4) DELETE `app_certs` rows, (5) DELETE app row. Order matters — revoke before file removal so the ACME server records the revocation when Caddy is reachable.
- **FR-018a**: Hard delete MUST be **resilient to Caddy / target unavailability** — steps 1-3 (Caddy interactions) MUST be wrapped in try/catch; failures MUST be logged at `warn` level (`logger.warn({ err, certId, step }, 'caddy cleanup failed during hard delete')` with structured context) and surface in the audit log (`app_cert_events` row with `event_type = 'hard_delete_partial'`, `event_data = { failed_step, error_message }`), but MUST NOT block steps 4-5. Rationale: if Caddy is down, the operator who triggered the delete still expects the row to disappear from the DB — ghost-app rows ("can't delete because the server is unreachable") are worse than orphaned cert files (which a daily reconciler picks up). Steps 4-5 are the source-of-truth cleanup; steps 1-3 are best-effort. The audit row signals to a future operator: "this app was hard-deleted but X cleanup steps failed; manual SSH check recommended."
