...
### Session 2026-04-28 (Gemini review pass)

- Q: Slug regex `^[a-z0-9]+(-[a-z0-9]+)*$` snaps non-ASCII repo names (`Мой-Супер-Проект` → empty, `🔥` → empty) to nothing → wizard crash. → A: **FR-006 derivation pipeline rewritten with NFD-decompose + diacritic strip + table-based transliteration + `repo-<sha256-prefix>` fallback for unmappable inputs.** The slug ALWAYS satisfies the regex; never returns empty. Adds a small npm dep (`transliteration` or equivalent table) — flagged for approval.
- Q: Compose `ports: ["${APP_PORT}:80"]` cannot be statically parsed; current FR-004 implies integer parsing → `TypeError`. → A: **FR-004 extended: parser flags ambiguous-port, leaves `upstream_port` UNSET, wizard prompts operator for the resolved value.** Never throws, never persists string-typed port, never tries to evaluate the env var (it belongs to the target compose runtime). YAML-malformed files block at Step 2 with parse-error context. Empty compose (no services) — same.
- Q: Idempotent CLONING retry uses `git fetch + reset --hard` — does NOT touch root-owned artefacts left by a previous failed compose run. → A: **FR-013 extended with `git clean -fdx`.** `-f` for unsafe deletes, `-d` for untracked dirs, `-x` for ignored files (build outputs, node_modules, mounted-volume detritus). Without it, retried bootstraps inherit invisible garbage from the previous attempt. Operators occasionally lose ad-hoc files in `${remote_path}` — that's the cost of clean retry; the contract is "anything not committed gets wiped on retry".
- Q: SSRF in `applications.healthUrl` — does this feature need to validate? → A: **No, lives in feature 006 (FR-029a/b).** Bootstrap takes Health Check URL via the same Add-Application path, which goes through 006's validator. Cross-feature linkage documented.

### Session 2026-04-28 (GPT review pass)

- Q: PAT expires or gets revoked DURING a bootstrap (clone succeeds, fetch fails on retry; or 2FA challenge appears). Currently surfaces as generic `failed_clone`. → A: **FR-016a — distinct error_code `failed_clone_pat_expired` triggered by 401/403 in stderr.** UI shows dedicated "Reconnect GitHub" deeplink. Generic `failed_clone` (network errors, malformed URL) keeps the existing UI; only PAT-related auth failures get the specific banner.
- Q: `composePath` accepts arbitrary string from operator — `../../../etc/passwd` would let `docker compose -f` read outside the repo. → A: **FR-020a — three-layer validation (form/API/runner) mirroring feature 007's `scriptPath` pattern.** Rules: printable ASCII, ≤256 chars, no `..`, no `\\`, no leading `/`, must end `.yml`/`.yaml`. Same TOCTOU defence as 007 — runner re-validates immediately before SSH-command construction.
- Q: Operator opens the Bootstrap wizard, picks repo, hits Submit, closes browser tab while clone is in progress. Backend continues, but operator has no way to find the in-flight bootstrap. → A: **FR-026a — Bootstrap History panel on Server detail.** Lists in-flight + recent bootstraps with "Resume monitoring" deeplink; clicking re-opens the wizard scoped to that app. The orchestrator was always browser-independent; this exposes that to operators.
- Q: Should `bootstrap_auto_retry` default to `true` to make bootstraps "self-healing"? → A: **No, default `false`.** Auto-retry on `failed_clone_pat_expired` would loop forever (PAT is still bad), spam logs, and burn GitHub rate-limit slots. Auto-retry is opt-in per-app for legitimate transient failures (network blips, target reboots). Stays as spec'd.
...
- **Compose `ports:` uses environment variable interpolation** (`["${APP_PORT:-3000}:80"]`): static parser cannot resolve `${APP_PORT}`. Per FR-004, the wizard surfaces an explicit prompt asking for the resolved port; bootstrap does NOT crash, does NOT silently default, does NOT shell out to evaluate the variable. Operator types the actual port; the answer becomes `applications.upstream_port`. The compose itself is left untouched (we don't rewrite the operator's repo).
- **Repo with non-ASCII name** (`🔥💯`, `Мой-Супер-Проект`, `日本語リポ`): slug derivation per FR-006 uses NFD-decomposition + diacritic strip (handles Latin-extended), table-based transliteration (handles Cyrillic / Greek / Hebrew), and a `repo-<sha256-prefix>` fallback for unmappable inputs (handles emoji, CJK without table coverage). The slug ALWAYS satisfies `^[a-z0-9]+(-[a-z0-9]+)*$` — no failure path leaks an empty string into the form. Operator sees the derived slug in the wizard and may override.
- **Empty compose** (file parses but no `services:` key, or `services: {}`): wizard blocks at Step 2 with "Compose has no services defined. Bootstrap requires at least one service to deploy." Operator either fixes the repo or aborts.
...
- **FR-004**: The compose file MUST be parsed for services with `expose:` or `ports:`. Detection rules:
  ...
  - **Parser MUST NOT crash on un-resolvable values.** A `ports:` entry like `"${APP_PORT}:80"` or `"${HOST_PORT:-3000}:${CONTAINER_PORT}"` cannot be statically evaluated (env vars unknown at parse time). On encountering a value that does not parse to an integer, the parser MUST: (a) flag the service as "ambiguous-port", (b) leave `upstream_port` UNSET in the proposal, (c) wizard surface explicit prompt: "Service `<name>` uses an environment variable for its port (`${APP_PORT}`). Enter the resolved port number manually." with a number input. NEVER throw `TypeError` / NEVER persist a string-typed port. NEVER attempt to interpolate the env var from the dashboard's environment — the variable belongs to the target compose runtime, not us.
  - Empty compose file (parses successfully but has zero services or `services:` key absent): treated as "0 services" branch above + warning "Compose has no services defined. Bootstrap requires at least one service."
  - YAML parse failure (malformed file): blocking error at Step 2 — "Compose file is not valid YAML: `<parse-error>`. Fix the file in the repo and retry."
- **FR-005**: The wizard MUST detect the repo's default branch via GitHub API and prefill the Advanced "Branch" field.
- **FR-006**: The slug for `name` and `remote_path` MUST be derived from the repo name. Derivation pipeline:
  1. **Transliterate** non-ASCII characters to closest ASCII via Unicode NFD decomposition + diacritic strip (e.g. `Café` → `Cafe`, `Россия` → `Rossiya` via library transliteration table). Cyrillic / Greek / Hebrew / Arabic etc. handled via `transliteration` npm dep OR equivalent table-based mapping. NEVER use `toLowerCase()` alone — it does not strip non-ASCII.
  2. Lowercase the result.
  3. Replace any remaining non-`[a-z0-9]` runs with single `-`.
  4. Trim leading / trailing `-`.
  5. Truncate to 64 chars.
  6. **Fallback**: if the resulting slug is empty (e.g. repo name was pure emoji `🔥💯`, or all characters had no transliteration mapping), the slug MUST be `repo-<short-hash>` where `<short-hash>` is the first 8 hex chars of `sha256(<original-repo-name>)`. Guarantees a valid slug for every conceivable input.
  Final slug MUST match `^[a-z0-9]+(-[a-z0-9]+)*$`. Operator-editable in the wizard's Advanced section.
...
- **FR-013**: Each step MUST be idempotent. Implementation conventions:
  - `CLONING`: if `${remote_path}/.git` exists with matching `remote.origin.url`, run `git fetch && git reset --hard origin/<branch> && git clean -fdx` instead of `git clone`. The `git clean -fdx` step is mandatory — it removes untracked files (build artefacts from a previously-failed compose-up, root-owned files docker created in mounted volumes, leftover `node_modules/`). Without it, retried bootstraps inherit garbage from the previous attempt and the failure mode is invisible.
...
- **FR-016a**: Clone failures with HTTP `401` or `403` from GitHub MUST be detected and classified as `pat_expired_or_revoked` distinct from generic `failed_clone`. Detection: `git clone` over HTTPS surfaces these via stderr (`fatal: Authentication failed for 'https://github.com/...'` for 401; `remote: Permission ... denied to ...` for 403). The runner MUST grep stderr for these patterns. Distinct error_code on the `applications.bootstrap_state` audit row MUST be `failed_clone_pat_expired`. UI MUST surface a dedicated banner: *"GitHub authentication failed. The connection's PAT may be expired, revoked, or have changed scopes. [Reconnect GitHub →]"*. The deeplink takes operator to feature 002's connection-edit screen with the same connection id pre-selected. After reconnection, the operator returns to the bootstrap page and clicks Retry — the new PAT is used. The bootstrap row is NOT auto-deleted; same idempotent-retry pattern as other failure modes. SSO challenge (org-enforced SSO not authorized for this PAT) — same surface, same recommendation.
...
- **FR-020a**: `composePath` (in Edit Config and at bootstrap submit) MUST be sanitised against path traversal — same defence-in-depth pattern as feature 007's `scriptPath` validator (FR-003 in spec 007). Rules: (a) printable ASCII only `^[\x20-\x7E]+$`, ≤ 256 chars; (b) MUST NOT contain `..` (literal `..` substring rejected, prevents `../../etc/passwd`); (c) MUST NOT contain `\\` (Linux targets, backslash is a Windows-typo signal); (d) MUST NOT begin with `/` (no absolute paths — relative-to-`remote_path` only); (e) MUST end with `.yml` or `.yaml`. Validation MUST happen at three layers: form (UX), API route (Zod refinement), and runner (just before constructing the SSH command — same TOCTOU-defence pattern as feature 007). On rejection: `400 INVALID_PARAMS` with `error_code = 'compose_path_unsafe'`.
...
- **FR-026a**: A "Bootstrap History" panel MUST exist (e.g. on the Server detail view, sub-tab) listing all bootstrapped apps with: app name, current `bootstrap_state`, `created_at`, last state-change time, and action buttons. In-flight bootstraps (states INIT through CERT_ISSUED) get a "Resume monitoring" button that re-opens the wizard scoped to that app — operator who closed the browser tab can return and watch progress. Failed bootstraps (`failed_*`) get "Retry / Edit Config / Hard Delete" buttons. ACTIVE bootstraps (terminal state) get "View App" deeplink. The panel is the answer to "I closed the wizard, where did my bootstrap go?" — the orchestrator never depended on the wizard, but the operator's eyes did. Default sort: in-flight first (by `created_at desc`), then failed, then ACTIVE.
