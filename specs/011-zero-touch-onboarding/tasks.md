...
- [ ] T017 [BE] [US1] Implement `devops-app/server/services/compatibility-probe.ts` per research.md R-010 — typed `probeCompatibility(serverId: string): Promise<CompatibilityReport>`. Single composite SSH command (heredoc inline) collecting key=value lines for SSH_OK/SUDO_NOPASSWD/USE_PTY/DOCKER/DISK_FREE_GB/SWAP/OS_FAMILY/OS_VERSION/ARCH. **Use `df -PBG /` (POSIX flag, prevents wrap on long mount-source names) per gemini #6.** Parser maps to per-check `pass/warn/fail` per the matrix in `research.md` § R-010 — auto-fixable-by-Initialise checks (Docker missing, swap absent, use_pty set) come back as `warn` not `fail` per github P0 #3. Each check carries `autoFixableByInitialise: boolean` per `contracts/api.md` § CompatibilityCheck. Typed inputs/outputs, no `as any`.
